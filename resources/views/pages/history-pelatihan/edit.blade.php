...
@section('title', 'Edit History Pelatihan')

@section('content')
<div class="d-flex justify-content-between py-3 mb-4">
    <h4 class="fw-bold "><span class="text-muted fw-light">History Pelatihan /</span> Edit History Pelatihan</h4>
</div>
<div class="card p-3">
    ...
    <div class="card-body">
        <form action="{{ route('history-pelatihan.update', $data->id) }}" method="POST" enctype="multipart/form-data">
            @if (count($errors) > 0)
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    <input type="text" id="nameWithTitle" class="form-control" placeholder="Enter NRP" list="mekanik"
                        name="mekanik_id" value="{{ $data->mekanik->nrp ?? '' }}" />
                    <datalist id="mekanik">
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    <input type="text" id="nameWithTitle" class="form-control" placeholder="Enter Instruktur"
                        list="instruktur" name="instruktur_id" value="{{ $data->instruktur->nrp ?? '' }}" />
                ...
                <div class="col-sm-10">
                    <select id="defaultSelect" class="form-select" name="site_id">
                        ...
                        <option value="{{ $role->id }}"
                            {{ old('site_id', $data->site_id ) == $role->id ? 'selected' : '' }}>
                            {{ $role->name }}
                        </option>
                        @endforeach
                    </select>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    <select id="defaultSelect" class="form-select" name="department_id">
                        ...
                        <option value="{{ $role->id }}"
                            {{ old('department_id', $data->department_id) == $role->id ? 'selected' : '' }}>
                            {{ $role->name }}
                        </option>
                        @endforeach
                    </select>
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    <input type="text" id="nameWithTitle" class="form-control" placeholder="Enter Pelatihan"
                        list="pelatihan" name="pelatihan_id" value="{{ $data->pelatihan->name ?? '' }}" />
            ...
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    <input type="text" id="nameWithTitle" class="form-control" placeholder="Enter Kompetensi"
                        name="sub_egi" value="{{ $data->sub_egi ?? '' }}" />
                </div>
            </div>
            <div class="row mb-3">
                ...
                <div class="col-sm-10">
                    <select id="defaultSelect" class="form-select" name="status">
                        <option value="Lulus" {{ old('status', $data->status) == 'Lulus' ? 'selected' : '' }}>Lulus
                        </option>
                        <option value="Tidak Lulus"
                            {{ old('status', $data->status) == 'Tidak Lulus' ? 'selected' : '' }}>Tidak Lulus</option>
                    </select>
                </div>
            </div>
